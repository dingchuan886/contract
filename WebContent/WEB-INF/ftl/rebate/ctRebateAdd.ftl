  ...
  <div class="well">
    <form action="${contract_ct_url}contract/tbConRebate/saveRebate" method="POST" id="rebateForm">
      ...
      <div class="m2">
        ...
        <h3>业务返利申请单</h3>
        <div class="m3">
          	<input type="hidden" name="con_id" value="${conCt.con_ct_id}" >
          	<input type="hidden" name="user_id" value="${conCt.user_id}" >
        	<input type="hidden" name="cus_name" value="${conCt.cus_name}" >
        	<input type="hidden" name="applyDate" value="${applyDate?string("yyyy-MM-dd")}" >
        	<input type="hidden" name="al_back" value="${conCt.al_rebate}">
          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb7">
            <tbody>
              <tr>
                <td>业务员：${conCt.user_name}</td>
                <td>申请日期：${applyDate?string("yyyy-MM-dd")}</td>
              </tr>
              <tr>
                <td>客户名称：${conCt.cus_name}</td>
                <td>销售合同号/销售订单号：${conCt.con_ct_id}</td>
              </tr>
              <tr>
              ...
              <tr>
                <td colspan="2">
                  <div class="m-sqd">
                    <ul>
                      <li><span>合同总金额：</span>${conCt.con_total_price}</li>
                      <li><span>其中高开：</span><input type="text" id="" name="con_high" placeholder="其中高开" class="p"></li>
                    </ul>
                    <ul>
                      <li><span>已返利金额：</span>${conCt.al_rebate}</li>
                    </ul>
                    <ul>
