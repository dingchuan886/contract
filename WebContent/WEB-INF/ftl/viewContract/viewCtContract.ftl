...
<html>
  ...
  <div class="well">
    ...
    <#if ctContract.user_id==userInfo.usercode && ctContract.con_state==0><input id="edit" name="" type="button" value="修改" onClick="updateContract()"></#if>
    <input name="" type="button" value="打印" onClick="toPrint();">
    <#if ctContract.con_state==5><input name="" type="button" data-toggle="modal" value="提交" onclick="subContract()"/></#if>
    <ul class="nav nav-tabs">
            <li class="active"><a href="#" onclick="javascript:window.location.href='${contract_ct_url}contract/myContract/viewCtContract?conId=${ctContract.con_ct_id}'" data-toggle="tab">查看合同</a></li>
            <#if (accCount>0)>
            <li class=""><a href="#" onclick="javascript:window.location.href='${contract_ct_url}contract/conAccount/viewAccount?conId=${ctContract.con_ct_id}'" data-toggle="tab">查看相关说明</a></li>
            </#if>
            <#if (billCount>0)>
            <li class=""><a href="#" onclick="javascript:window.location.href='${contract_ct_url}contract/tbConBill/viewBills?id=${ctContract.con_ct_id}'" data-toggle="tab" >查看开票</a></li>
            </#if>
            <#if (rebateCount>0)>
            <li class=""><a href="#" onclick="javascript:window.location.href='${contract_ct_url}contract/tbConRebate/rebateBill?conId=${ctContract.con_ct_id}'" data-toggle="tab">查看返利</a></li>
   			</#if>
    </ul>
    </div>
    <div class="logo">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb3">
          <tbody>
            <tr>
              ...
              <input type="checkbox" checked="checked" disabled="true">团购</label><label style="margin:0;float:left"><input type="checkbox" disabled="true">特卖惠</label></td>
              </#if>
              <#if ctContract.bus_type==2>
              <input type="checkbox" disabled="true">团购</label><label style="margin:0;float:left"><input type="checkbox" checked="checked" disabled="true">特卖惠</label></td>
              </#if>
            </tr>
            <tr>
