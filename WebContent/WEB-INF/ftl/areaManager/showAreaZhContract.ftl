  ...
  <body>
	 ...
	 <div class="span10">
	 	<ul class="nav nav-tabs">
            ...
            <li class="active"><a href="javascript:void(0)" onclick="javascript:window.location.href='${contract_ct_url}contract/areaManager/showZhareaManager'" data-toggle="tab">众智315广告合同</a></li>
            <li class=""><a href="javascript:void(0)" onclick="javascript:window.location.href='${contract_ct_url}contract/areaManager/showCtareaManager'" data-toggle="tab">车团合同</a></li>
          </ul>
          <form action="/contract/areaManager/showZhareaManager" class="form-horizontal" id="form1">
            <div class="row-fluid">
            ...
            <div class="row-fluid">
              ...
              <div class="control-group span5">
               ...
               <div><button onclick="doSubmit('/contract/areaManager/showZhareaManager')">查询</button>
               	<button onclick="doSubmit('/contract/downloadAttach/downloadDeptZhExcell')">下载数据</button></div>
            </div>
            ...
            <table class="table table-bordered">
              ...
              <#if zh.conZh.con_state != 5>
                <tr <#if zh_index%2==0>style="background-color:#FDD8C6"<#else>style="background-color:#CBE4E8"</#if>>
                  ...
                  <td>
                  <#list zh.supZh as addCon>
                  	  <#if addCon_index==(zh.supZh?size-1)>
                  		${addCon.exe_date}
                  	  <#else>
                  		${addCon.exe_date}<hr color="black" SIZE="3"/>
                  	  </#if>
                  	</#list>
                  </td>
                  <td>${zh.conZh.con_total_price}</td>
                  <td>
                  	${zh.supZh[0].exe_price}
                  </td>
                 <#--开票日期 -->
                 <td>
                 ...
                 <#if (zh.conRebates?size>0)>
                  ...
                  <#if zh.conZh.con_state==3><a href="javascript:void(0)" onclick="down(this)">下载附件</a></#if>
                  <#if zh.conZh.con_state==3><a href="javascript:void(0)" onclick="preView(this)">预览附件</a></#if>
                  </td>
                </tr>
                </#if>
                </#list>
              ...
              <td></td>
              <td></td>
              <td id="conTotal">${allZhMoney.conMoney}</td>
              <td>${allZhMoney.actMoney}</td>
              <td></td>
              <td id="billMoney">${allZhMoney.billMoney}</td>
              <td></td>
              <td id="backMoney">${allZhMoney.rmMoney}</td>
              <td id="rebateMoney">${allZhMoney.rebateMoney}</td>
              <td></td>
              </tr>
              </tbody>
            </table>
	...
	<div align="right">
		...
		<a href="javascript:void(0)" onclick="changePage(${deptZhContract.pageNum-1})">上一页</a>&nbsp
		第${deptZhContract.pageNum}/${deptZhContract.totalPages}页		
		<a href="javascript:void(0)" onclick="changePage(${deptZhContract.pageNum+1})">下一页</a>&nbsp&nbsp		
		<a href="javascript:void(0)" onclick="changePage(${deptZhContract.totalPages})">尾页</a>&nbsp&nbsp		
		<input type="text" id="pageNum" value="${deptZhContract.pageNum}" style="width:25px;"/><button type="button" onclick="changePage1()">跳页</button>&nbsp	&nbsp&nbsp		
...
$(document).ready(function(){
	...
	<#list deptZhContract.list as zh>
		<#if zh.conZh.con_state != 5>
			conTotal[${zh_index}] = ${zh.conZh.con_total_price};
		</#if>
	</#list>
	...
	*/
	...
	if(pageNum>${deptZhContract.totalPages}){
		pageNum = ${deptZhContract.totalPages};
	}
...
function viewCon(obj){
	var id = $(obj).parent().siblings("td").eq(1).html();
	window.open("/contract/flowViewContract/viewZhContract?conId="+id);
}
//修改合同
...
function delCon(obj){
	...
	var id = $(obj).parent().siblings("td").eq(1).html();
	var pageNum = ${deptZhContract.pageNum};
	if(flag){
