  ...
  <body>
	 ...
	 <div class="span10">
	 	<ul class="nav nav-tabs">
	 		<!-- to mycontract -->
            <li class=""><a href="javascript:void(0)" onclick="javascript:window.location.href='${contract_ct_url}contract/areaManager/showCzareaManager'" data-toggle="tab">众智车展合同</a></li>
            <li class=""><a href="javascript:void(0)" onclick="javascript:window.location.href='${contract_ct_url}contract/areaManager/showZhareaManager'" data-toggle="tab">众智315广告合同</a></li>
            <li class="active"><a href="javascript:void(0)" onclick="javascript:window.location.href='${contract_ct_url}contract/areaManager/showCtareaManager'" data-toggle="tab">车团合同</a></li>
          </ul>
          <form action="/contract/areaManager/showCtareaManager" class="form-horizontal" id="form1">
            <div class="row-fluid">
            ...
            <div class="row-fluid">
              <div class="control-group span5">
                ...
                <div class="controls">
                  <select name="conType">
                  	<option value=""></option>
                  	<option value="1" <#if pageCondition.conType==1>selected</#if>>团购</option>
                  	<option value="2" <#if pageCondition.conType==2>selected</#if>>特卖惠</option>
                  </select>
                </div>
              </div>
              <div class="control-group span5">
            ...
            <div class="row-fluid">
              ...
              <div class="control-group span5">
               ...
               <div><button onclick="doSubmit('/contract/areaManager/showCtareaManager')">查询</button>
               	<button onclick="doSubmit('/contract/downloadAttach/downloadDeptCtExcell')">下载数据</button>
               </div>
            </div>
          ...
          <div class="bs-docs-example" style="width:100%; overflow-x:scroll;white-space:nowrap;">
            <table class="table table-bordered">
              ...
              <tbody>
              <#list deptCtContract.list as ct>
              	<#if ct.conCt.con_state != 5>
                <tr <#if ct_index%2==0>style="background-color:#FDD8C6"<#else>style="background-color:#CBE4E8"</#if>>
                  <td>
                  ${ct_index+1+(deptCtContract.pageNum-1)*10}
                  </td>
                  <td>${ct.conCt.con_ct_id}</td>
                  <td><#if ct.conCt.con_state==0>
                  	等待审核
                  </#if>
                  <#if ct.conCt.con_state==1>
                  	经理已审核
                  </#if>
                  <#if ct.conCt.con_state==2>
                  	流程部已审核
                  </#if>
                  <#if ct.conCt.con_state==3>
                  	流程部已归档
                  </#if>
                  <#if ct.conCt.con_state==4>
                  	已驳回
                  </#if>
                  <#if ct.conCt.con_state==7>
                  	区域经理已审核
                  </#if>
                  </td>
                  <td>${ct.orgName}${ct.deptName}</td>
                  <td>${ct.conCt.user_name}</td>
                   <td>${ct.conCt.create?string("yyyy-MM-dd")}</td>
                  <td>${ct.conCt.cus_name}</td>
                  <td>${ct.conCt.cus_brand}</td>
                  <td>${ct.conCt.cus_tel}</td>
                  <td><#if ct.conCt.con_state==3>是
                  	<#else>
                  	否
                  </#if>
                  </td>
                  <td>${ct.conCt.total_price}</td>
                  <td>${ct.supCt.car_count}</td>
                  <td>${ct.supCt.con_total_price}</td>
                  <td>${ct.supCt.exe_price}</td>
                  <td>${ct.conCt.act_date?string("yyyy-MM-dd")}</td>
                  <td>${ct.supCt.exe_date}</td>
                     <#--开票日期 -->
                 <td>
                 <#if (ct.conBills?size>0)>
                 	<#list ct.conBills as bill>
                 	<#if (bill_index<(ct.conBills?size-1))>
                 		<span>${bill.create?string("yyyy-MM-dd")}</span><hr color="black" SIZE="3"/>
              				<#elseif bill_index==(ct.conBills?size-1)>
                 ...
                 <#--返利金额 -->
                 <td>
                 <#if (ct.conRebates?size>0)>
                 	<#list ct.conRebates as rebate>
                 	<#if (rebate_index<(ct.conRebates?size-1))>
                 		<span>${rebate.back_actual}</span><hr color="black" SIZE="3"/>
              				<#elseif rebate_index==(ct.conRebates?size-1)>
                  ...
                  <#if ct.conCt.con_state==3><a href="javascript:void(0)" onclick="down(this)">下载附件</a></#if>
                  <#if ct.conCt.con_state==3><a href="javascript:void(0)" onclick="preView(this)">预览附件</a></#if>
                  </td>
                </tr>
                </#if>
                </#list>
              ...
              <td></td>
              <td></td>
              <td>${allCtMoney.conMoney}</td>
              <td id="conTotal">${allCtMoney.actMoney}</td>
              <td></td>
              <td></td>
              <td></td>
              <td id="billMoney">${allCtMoney.billMoney}</td>
              <td></td>
              <td id="backMoney">${allCtMoney.rmMoney}</td>
              <td id="rebateMoney">${allCtMoney.rebateMoney}</td>
              <td></td>
              </tr>
              </tbody>
            </table>
	...
	<div align="right">
		...
		<a href="javascript:void(0)" onclick="changePage(${deptCtContract.pageNum-1})">上一页</a>&nbsp
		第${deptCtContract.pageNum}/${deptCtContract.totalPages}页		
		<a href="javascript:void(0)" onclick="changePage(${deptCtContract.pageNum+1})">下一页</a>&nbsp&nbsp		
		<a href="javascript:void(0)" onclick="changePage(${deptCtContract.totalPages})">尾页</a>&nbsp&nbsp		
		<input type="text" id="pageNum" value="${deptCtContract.pageNum}" style="width:25px;"/><button type="button" onclick="changePage1()">跳页</button>&nbsp	&nbsp&nbsp		
...
$(document).ready(function(){
	...
	$.post("/contract/brandAndSeries/getAllOrg",function(data){
		...
		for(var i=0;i<suc.length;i++){
			if(suc[i].org_name==org){
				$("#orgName").append("<option value='"+suc[i].org_name+"' selected>"+suc[i].org_name+"</option>");
			}else{
				$("#orgName").append("<option value='"+suc[i].org_name+"'>"+suc[i].org_name+"</option>");
			}
		}
	});
...
function changePage(pageNum){
	...
	if(pageNum>${deptCtContract.totalPages}){
		pageNum = ${deptCtContract.totalPages};
	}
...
function showFlow(data){
	...
	$.ajax({
		url:"/contract/myContract/showCtFlow",
		type:"GET",
		data:{"ctContractId":id},
...
function viewCon(obj){
	var id = $(obj).parent().siblings("td").eq(1).html();
	window.open("/contract/flowViewContract/viewCtContract?conId="+id);
}
//修改合同
function editCon(obj){
	var id = $(obj).parent().siblings("td").eq(1).html();
	window.open("/contract/ctContract/toupdateCtContract?conId="+id);
}
//提交合同
function subCon(obj){
	var id = $(obj).parent().siblings("td").eq(1).html();
	window.open("/contract/ctContract/viewCtContract?conId="+id);
}
//删除合同
function delCon(obj){
	...
	var id = $(obj).parent().siblings("td").eq(1).html();
	var pageNum = ${deptCtContract.pageNum};
	if(flag){
	location.href="/contract/ctContract/delCtContract?conId="+id+"&pageNum="+pageNum;
	}
}
